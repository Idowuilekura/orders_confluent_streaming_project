...
file_name = os.path.join(os.path.dirname(__file__), 'shopileft_producer_demo.log')
print(file_name)
def test_this():
    err = 'error'
    print(file_name)
    logging.basicConfig(filename=file_name, level=logging.INFO, filemode='w', format='%(asctime)s - %(levelname)s - %(message)s', force=True)
