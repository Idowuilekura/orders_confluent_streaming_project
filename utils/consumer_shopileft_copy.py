...
def call_my_pool(value):
    con = mypool.connect()
    print(f'closing the pool  {value}')
    con.close()
    

with Pool(10) as p:
    p.map(call_my_pool, range(10))
...
#                 create_insert_data_(value,table_name_transaction=table_name_transaction,table_name_analytics=table_name_analytical,database_conn=con,mypool=mypool)
#                 consumer.commit()
